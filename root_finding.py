def sequential(f, x_init, x_interval, epsilon, x_final):
    ...
    # i+1 번째 x 와 i 번째 x 의 간격이 너무 작지는 않은지 확인
    # check if the interval between i+1st x and i-th x is too small
    assert epsilon < abs(x_interval), x_interval

    # 일련의 x_i 값을 미리 준비한다
    # Prepare a series of x_i values in advance
    n = int((x_final - x_init) / x_interval) + 1
